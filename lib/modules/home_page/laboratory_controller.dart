...
class LaboratoryController extends GetxController {
  var laboratoryModel = <LaboratoryModel>[].obs;
  var isLoading = false.obs;
  final MyLanguageController languageController = Get.find();

  Future<void> fetchLaboratory(int id) async {
    isLoading.value = true;
    final box = GetStorage();
    final cacheKey = 'laboratoryModel_$id';
    final cacheTimeKey = 'laboratoryModel_last_fetch_$id';

    final cachedData = box.read(cacheKey);
    if (cachedData != null) {
      try {
        List jsonData = json.decode(cachedData);
        laboratoryModel.value =
            jsonData.map((e) => LaboratoryModel.fromJson(e)).toList();
      } catch (e) {
    ...
    if (shouldFetchFromApi) {
      try {
        final response =
        await HttpHelper.gettData(url: 'get/medbylaboratory_id/$id');
